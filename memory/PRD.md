...
## Update (2026-06) — Bulk game import from .lua files (App ID + Steam title auto-resolve)
- USER GOAL (clarified): on Add Game, besides the one-by-one form, an option to drop MANY .lua files at once. Each .lua is named by the Steam App ID (e.g., 3321460.lua). System creates a game per file: app_id = filename stem, attaches the .lua content, and AUTO-RESOLVES the title from the App ID via Steam. Covers added later one by one.
- BACKEND: `resolve_steam_title(app_id)` calls Steam `store.steampowered.com/api/appdetails?appids=<id>&filters=basic` (urllib, 6s timeout, best-effort; pod CAN reach Steam, ~0.2s). New `POST /api/games/bulk-lua` (multipart `files: list[UploadFile]`) → per file: app_id from filename, upload lua to Storage, create game (title=resolved or app_id fallback), return {created:[{id,app_id,title,resolved}], count}. Verified via curl (2 files → correct app_ids + resolved titles; test games deleted).
- ADMIN (GameManage add mode): new "Importar vários jogos por arquivos .lua" card (data-testid bulk-import-card, bulk-lua-input, multiple, accept .lua) with results list (app_id → title, resolved/manual badge, edit link) and an "OU ADICIONE UM POR UM" divider above the existing single-add form. i18n game.bulk* (pt/en/es). Verified via screenshot.
- Also kept: per-game multi-.lua upload (input multiple) + game.uploadMultiHint on the manage page.
- NOTE: title resolution may include suffixes (e.g., 3321460 → "Crimson Desert Enhanced") which can differ from the bypass name; user can edit the title so the by-name bypass link matches.
- IMPORTANT: these are Admin(React web) + backend changes → they are on PREVIEW now; user must click REPUBLISH to get them on the production Admin/backend (production /api/games/bulk-lua won't exist until republish).

## Update (2026-06) — Affiliate system (roles, scoped keys)
- USER DECISIONS: affiliate logs in with ONLY an AFF key (key = credential); same login page (email/pwd=admin, AFF-*=affiliate); affiliate-created client keys use same fields (name/email/phone); admin sees all keys + filter by affiliate; deleting an affiliate keeps their client keys working.
- Consulted integration_expert (JWT two-role playbook). Implementation kept consistent with existing plaintext access_keys (affiliate key stored plaintext for admin re-display; documented pragmatic exception vs the hash+pepper ideal).
- SQL (user ran on their Supabase — shared by preview+prod): `backend/supabase_affiliates.sql` — table `affiliates`(id,name,key unique,active,created_at) + `access_keys.affiliate_id` + `access_keys.affiliate_name`.
- BACKEND (auth.py): create_affiliate_token + decode_token. (server.py): current_principal dependency (admin or active affiliate; re-checks affiliates table for active each request → immediate disable on delete). Login detects AFF-* / affiliate_key → affiliate token. /auth/me returns principal. New admin-only /affiliates GET/POST/DELETE (_gen_aff_key = AFF-XXXX-XXXX-XXXX). /keys create/list/delete/reset now scoped: affiliate → own affiliate_id only (404 on others), admin → all + optional ?affiliate_id filter; keys tagged with affiliate_id+affiliate_name. validate_key unchanged (keys keep working regardless of affiliate).
- FRONTEND: Login.js accepts AFF key (password hidden in aff mode) → stores role/principal_name → routes affiliate to /keys. Layout.js role-based nav (affiliate sees ONLY Keys + redirect guard + footer "Área do afiliado · <name>"). Keys.js: admin gets Affiliates management card (create/copy/delete AFF key) + affiliate filter + affiliate_name badge per key; affiliate sees only creation form + own keys. i18n aff.* + login.affHint + nav.affArea (pt/en/es).
- VERIFIED via curl + screenshot: admin creates Luan/Caio; each logs in by key; isolation (Luan sees 1, Caio sees 1); admin sees all + affiliate_name; cross-affiliate delete/reset → 404; affiliate UI shows only Keys nav, no affiliates section. Test data cleaned up.
- IMPORTANT: backend+Admin code is on PREVIEW; user must REPUBLISH for the affiliate system to work on the production Admin.

## Update (2026-06) — Steam auto close/reopen + key-revoke self-wipe (client)
- FEATURE 1 (native, main.js): install-dependencies now detects Steam via `tasklist` (steam.exe), `taskkill /IM steam.exe /F /T` before installing (avoids EBUSY DLL locks), waits 2.5s, installs, then relaunches steam.exe from getSteamPath() (spawn detached). Returns steamReopened. Renderer shows "Fechando a Steam…" (dep-progress sentinel filename "steam:closing") and a "Steam reaberta" toast (i18n t.steamClosing/t.steamReopened pt/en/es). Needs client re-download (native).
- FEATURE 2 (renderer, served remotely): when the stored key is deleted/revoked/reused-elsewhere, the client auto-wipes. gate() background-revalidates the stored key on launch; if invalid and reason != "network" → revokeAndWipe(): calls window.api.deleteAll() (removes installed deps + injected game files), clears stored key, shows blocked GATE msg gt("revoked"). Also startKeyWatch() re-checks every 5 min during the session and wipes+blocks if revoked mid-use. Server validate_key unchanged (returns not_found/revoked/other_device). No push possible: wipe happens on the client's next launch or within 5 min if open.
- Client .exe rebuilt + re-zipped (icon embedded). Renderer changes syntax-verified; Feature 2 not runtime-tested on Windows (needs a real client with installed files + a deleted key).
- DEPLOY NUANCE: distributed client points to PRODUCTION (config apiBase=emergent.host) which serves its last-published snapshot of /api/client-app (renderer) and /api/client-build (zip). So both features reach real clients only after REPUBLISH (renderer for auto-wipe; new zip for the Steam native feature). Preview already has both for testing.

## Update (2026-06) — NATIVE self-updater (no more re-downloads) + .lua removal fix
- USER GOAL: client should auto-update its NATIVE code too (like Admin), so after a Republish old clients update themselves with no re-download.
- ARCHITECTURE: split the desktop app into a fixed loader + swappable payload.
  - `/app/desktop/bootstrap.js` = Electron `main` entry (baked in the .exe, must stay stable). On launch it reads root `config.json` apiBase, fetches `GET /api/client-native/bundle.json` (12s timeout), and if `version` > active, downloads all files (base64) into `userData/native/next/`, promotes to `userData/native/current/`, then `require()`s `main.js` from the active dir (current if newer/equal, else bundled). Offline/any error → keeps current. Never runs tests; pure JS.
  - Native code moved to `/app/desktop/app/` (`main.js`, `preload.js`, `config.json`, `renderer/`, `version.json`). node-machine-id REMOVED — HWID now via inline `reg query MachineGuid` (payload is pure JS, no node_modules needed).
  - package.json: main=bootstrap.js, files=[bootstrap.js, config.json, app/**], version 1.0.2.
- BACKEND: `GET /api/client-native/bundle.json` → {version (from app/version.json), files:{relpath:base64}} built from /app/desktop/app/ (excludes version.json). CLIENT_APP_DIR moved to /app/desktop/app/renderer (remote UI mount unchanged path /api/client-app). Verified: bundle returns v1.0.2 + 7 files; /api/client-app 200.
- .LUA REMOVAL FIX (in this build): deactivate-game and delete-all now close Steam first (unlink was silently failing on EBUSY because Steam locked the .lua), unlinkRobust retries 4x, then reopen Steam → game disappears from Steam library. delete-all also used by revoke self-wipe.
- WORKFLOW GOING FORWARD: change native code in /app/desktop/app/** → bump /app/desktop/app/version.json → REPUBLISH → old clients auto-update on next launch (no download). The .exe (bootstrap) only needs re-download if bootstrap.js itself changes (rare). THIS build is the last mandatory re-download to get the loader.
- Not runtime-tested on Windows (per user request: no automated tests). Syntax-checked; asar verified to contain bootstrap.js + app/main.js + app/renderer.

## Update (2026-06) — Update loader at key-entry + version 1.0.3
- app/main.js: added `check-native-update` IPC (+ _verOf/_cmp/fetchBundleAndPromote) — fetches /api/client-native/bundle.json, if newer downloads→promotes userData/native/current, then app.relaunch()+exit. preload exposes checkNativeUpdate. renderer init() now shows "Verificando atualização…" loader and calls checkNativeUpdate() before splash+gate (so the client pulls the latest native code right when opening, then continues to the key screen). This complements the bootstrap.js startup check.
